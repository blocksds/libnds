...
// of a circle, cause its enough to calculate the rest with it.
...
// A libnds angle has 32768 degrees (16 bits long, signed) with an range of
// [-32768 - 32767] (32768, 0 and -32768 al describe the same angle).
#define LIBNDS_ANGLE_BITS 15
#define LIBNDS_ANGLE      (1 << LIBNDS_ANGLE_BITS)
#define LIBNDS_ANGLE_MASK (LIBNDS_ANGLE - 1)

// The number of bits that is used by a libnds angle but gets chopped off with a
// LUT angle.
...
// When getting an libnds angle, you should first convert it to a 15 bit
// unsigned number, so you only have a positive angle and it doesn't affect the
// outcome (-270 degree == 90 degree).
#define TO_POSITIVE_ANGLE(angle) (((angle) < 0) ? (LIBNDS_ANGLE + (angle)) : (angle))

// To convert a libnds angle to a LUT angle, you must shift it right so it fits
// in a LUT angle and mask of any bits that doesn't fit:
...
const u16 SIN_LUT[LUT_SIZE + 1] = {
    ...
    18204, 18537, 18868, 19195, 19519, 19841, 20159, 20475, 20787, 21097, 21403, 21706,
    22005, 22301, 22594, 22884, 23170, 23453, 23732, 24007, 24279, 24547, 24812, 25073,
    25330, 25583, 25832, 26077, 26319, 26557, 26790, 27020, 27245, 27466, 27684, 27897,
    28106, 28310, 28511, 28707, 28898, 29086, 29269, 29447, 29621, 29791, 29956, 30117,
    30273, 30425, 30572, 30714, 30852, 30985, 31114, 31237, 31357, 31471, 31581, 31685,
    31785, 31881, 31971, 32057, 32138, 32214, 32285, 32351, 32413, 32469, 32521, 32568,
...
const s32 TAN_LUT[LUT_SIZE + 1] = {
    ...
    25280,  26208,  27145,  28092,   29050,   30017,   30996,   31986,   32988,  34002,
    35029,  36070,  37125,  38195,   39280,   40382,   41500,   42635,   43789,  44962,
    46155,  47369,  48604,  49862,   51144,   52451,   53784,   55143,   56531,  57949,
    59398,  60879,  62395,  63946,   65535,   67164,   68834,   70548,   72307,  74115,
    75974,  77886,  79855,  81884,   83976,   86135,   88365,   90669,   93053,  95522,
    98081,  100736, 103492, 106358,  109340,  112446,  115687,  119071,  122609, 126313,
    130197, 134275, 138564, 143081,  147846,  152883,  158217,  163877,  169895, 176309,
    183160, 190499, 198380, 206869,  216043,  225990,  236816,  248647,  261634, 275959,
    291845, 309567, 329471, 351993,  377693,  407305,  441807,  482534,  531351, 590957,
    665398, 761030, 888449, 1066729, 1334015, 1779313, 2669640, 5340085, MAX_TAN
};
...
s32 sinLutLookup(int i)
{
    ...
    if (i < 128)
        return SIN_LUT[lutVal]; // first quarter
    if (i < 256)
        return SIN_LUT[LUT_SIZE - lutVal]; // second quarter
    if (i < 384)
        return -SIN_LUT[lutVal];        // thirth quarter
...
{
    // Convert from libnds to LUT angle, and making sure the angle is in the
...
s16 asinLerp(s16 par)
{
    ...
    u16 *lutIndexPointer =
        (u16 *)bsearch(&param, SIN_LUT, LUT_SIZE + 1, sizeof(u16), asinComp);
