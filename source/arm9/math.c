...
ARM_CODE float hw_rsqrtf(float x)
{
	...
	exponent=-exponent;
        ...
        while (REG_SQRTCNT & SQRT_BUSY);
        // waits for square root operation to complete
        u32 sqrtres = REG_SQRT_RESULT<<4;
	while(REG_DIVCNT & DIV_BUSY);
	u32 divres = REG_DIV_RESULT_L<<5;
	u32 new_mantissa=(((u64)sqrtres*(u64) divres)>>32);
        new_mantissa += 1; // necessary for rounding
        new_mantissa >>= 2;// shift down after rounding
        // construct float from exponent and mantissa
        xu.i = exponent | (new_mantissa & ((1 << 23) - 1));
        return xu.f;
    }
