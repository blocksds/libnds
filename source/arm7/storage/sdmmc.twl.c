...
//  if (readdata)
//      sdmmc_mask16(REG_DATACTL32, 0x1000, 0x800);
//  if (writedata)
//      sdmmc_mask16(REG_DATACTL32, 0x800, 0x1000);
...
{
    if (!force_init && sdmmc_controller_initialised)
        return;
...
int sdmmcMsgHandler(int bytes, void *user_data, FifoMessage *msg)
{
    ...
    switch (msg->type)
    {
        case SDMMC_SD_READ_SECTORS:
            retval = sdmmc_readsectors(&deviceSD, msg->sdParams.startsector,
                                       msg->sdParams.numsectors, msg->sdParams.buffer);
            break;
        case SDMMC_SD_WRITE_SECTORS:
            retval = sdmmc_writesectors(&deviceSD, msg->sdParams.startsector,
                                        msg->sdParams.numsectors, msg->sdParams.buffer);
            break;
        case SDMMC_NAND_READ_SECTORS:
            retval = sdmmc_readsectors(&deviceNAND, msg->sdParams.startsector,
                                       msg->sdParams.numsectors, msg->sdParams.buffer);
            break;
        case SDMMC_NAND_WRITE_SECTORS:
            retval = sdmmc_writesectors(&deviceNAND, msg->sdParams.startsector,
                                        msg->sdParams.numsectors, msg->sdParams.buffer);
            break;
    }
