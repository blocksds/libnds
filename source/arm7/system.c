...
u32 getBatteryLevel(void)
{
    ...
    if (!isDSiMode())
    {
        // This code reads the DS-specific registers and generates a byte with
        // values that look like the ones read from the DSi battery status
        // register.

        // Battery Status. If bit 0 is set, the battery has low charge (red). In
        ...
        // In NDS (and DSi in NDS mode) bit 6 is 1. In DSi in DSi mode bit 6 is
        // zero.
        if (backlight & (1 << 6))
        {
...
u32 systemSetBacklightLevel(u32 level)
{
    ...
    if (backlight_level & 0xF0)
    {
        ...
        if (level < (PM_BACKLIGHT_MIN + 1))
            level = PM_BACKLIGHT_MIN + 1;

        u8 reg = readPowerManagement(PM_BACKLIGHT_LEVEL);
        reg = (reg & ~0x03) | (level - (PM_BACKLIGHT_MIN + 1));
        writePowerManagement(PM_BACKLIGHT_LEVEL, reg); // 0 to 3
        return level;
    }
    else
    {
        return PM_BACKLIGHT_MAX;
    }
}
